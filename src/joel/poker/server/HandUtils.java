...
public class HandUtils {
    ...
    public static Hand fromCards(Card[] cards) {
        ...
        for (int i = 0; i < cardCount.length; i++) if (cardCount[i] != 0) System.out.println(Card.CardValue.fromID(i).toString() + " - " + cardCount[i]);

        System.out.println("\nSuitCount:");
        for (int i = 0; i < suitCount.length; i++) if (suitCount[i] != 0) System.out.println(Card.CardSuit.fromID(i).toString() + " - " + suitCount[i]);
        ...
        for (int i = 0; i < cards.length; i++) {
            ...
            if (nextCardsFound == 3) {
                ...
                if (high.getValue() == Card.CardValue.FIVE) lastValue = Card.CardValue.ACE.getValue();
        ...
        for (int i = 0; i < cards.length; i++) {
            Card.CardValue value = cards[i].getValue();
            int amount = 0;
            for (int j = 0; j < cards.length; j++) {
                Card c = cards[j];
            ...
            if (amount == 4) { // has 4 of kind, determine kicker
                Card.CardValue kicker = Card.CardValue.TWO;
                for (int j = 0; j < cards.length; j++) {
                    Card c = cards[j];
        ...
        if (three != -1 && two != -1) return new FullHouseHand(Card.CardValue.fromID(three), Card.CardValue.fromID(two));

        // Check for flush

        for (int i = 0; i < suitCount.length; i++) {
            if (suitCount[i] >= 5) {
                Card.CardSuit suit = Card.CardSuit.fromID(i);
                Card.CardValue high = Card.CardValue.TWO;
                for (int j = 0; j < cards.length; j++) {
                    Card c = cards[j];
        ...
        for (int i = 0; i < cards.length; i++) {
            Card.CardValue high = cards[i].getValue();
            ...
            if (nextCardsFound == 3) {
                ...
                if (high == Card.CardValue.FIVE) lastValue = Card.CardValue.ACE.getValue();
        ...
        if (three != -1) { // has 3 of kind, determine 2 kickers
            Card.CardValue threeValue = Card.CardValue.fromID(three);
            Card.CardValue kicker1 = Card.CardValue.TWO, kicker2 = Card.CardValue.TWO; // kicker1 >= kicker2
            for (int i = 0; i < cards.length; i++) {
                Card c = cards[i];
        ...
        if (pair1 != -1 && pair2 != -1) {
            Card.CardValue pair1Value = Card.CardValue.fromID(pair1);
            Card.CardValue pair2Value = Card.CardValue.fromID(pair2);
            Card.CardValue sortedValue1, sortedValue2;
            ...
            Card.CardValue pair3Value;
            if (pair3 != -1) {
                pair3Value = Card.CardValue.fromID(pair3);
        ...
        if (pair != -1) {
            Card.CardValue pairValue = Card.CardValue.fromID(pair);
            Card.CardValue kicker1 = Card.CardValue.TWO, kicker2 = Card.CardValue.TWO, kicker3 = Card.CardValue.TWO;
            for (int i = 0; i < cards.length; i++) {
                Card c = cards[i];
        ...
        Card.CardValue[] best = new Card.CardValue[5];
        for (int i = 0; i < best.length; i++) best[i] = Card.CardValue.TWO;
        for (int i = 0; i < cards.length; i++) {
            Card.CardValue c = cards[i].getValue();
